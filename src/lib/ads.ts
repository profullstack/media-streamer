 ...
 * deployment writes .env from one opaque GitHub secret, so leaving it to the
 * environment meant nobody could add it without rewriting that whole blob.
 *
 * AD_SLOT still overrides, which is what a staging copy pointing at a test
 * slot would use. Setting it to an empty string turns adverts off.
 */
