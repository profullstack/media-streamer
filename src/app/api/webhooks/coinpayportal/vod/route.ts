/**
 * CoinPayPortal webhook — VOD monetization.
 *
 * Verifies the signature, then flips the matching VOD grant to `paid` (weekly
 * pass / per-title stream get a windowed expiry; a per-title *download* is
 * permanent) or `expired`. Idempotent on the CoinPay payment id. Mirrors the
 * signature-verifying pattern of the subscription webhook.
 *
 * @route POST /api/webhooks/coinpayportal/vod
 * @route GET  /api/webhooks/coinpayportal/vod
 */

import { NextRequest, NextResponse } from 'next/server';

import { getCoinPayPortalClient } from '@/lib/coinpayportal/client';
import { handleVodWebhook } from '@/lib/vod';

const LOG_PREFIX = '[Webhook:VOD]';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (metadata && metadata.type && metadata.type !== 'vod') {
    return NextResponse.json({ success: true, action: 'ignored:not_vod' }, { status: 200 });
  }

  try {
    const outcome = await handleVodWebhook({
      type,
      paymentId,
    ...
    console.log(`${LOG_PREFIX} ${outcome.action}`, { paymentId, grantId: outcome.grantId });
    return NextResponse.json({ success: true, action: outcome.action, grantId: outcome.grantId }, { status: 200 });
  } catch (error) {
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json({ status: 'ok', endpoint: '/api/webhooks/coinpayportal/vod' });
}
