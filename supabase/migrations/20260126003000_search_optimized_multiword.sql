-- Optimized multi-word search
-- Use first word to leverage GIN index, then filter remaining words
...
BEGIN
    ...
    search_words := array_remove(string_to_array(normalized_query, ' '), '');

    -- Get first word for indexed lookup
    first_word := search_words[1];
    first_pattern := '%' || first_word || '%';
    ...
    user_results AS (
        ...
        FROM bt_torrents bt
        WHERE
            bt.name ILIKE first_pattern
            OR bt.clean_title ILIKE first_pattern
    ),
    -- Filter user results for all words
    user_filtered AS (
        SELECT ur.*
        FROM user_results ur
        WHERE NOT EXISTS (
            SELECT 1 FROM unnest(search_words) AS w
            WHERE ur.name NOT ILIKE '%' || w || '%'
        )
    ),
    -- DHT torrents - use first word for index, limit early
    dht_candidates AS (
        SELECT
            t.info_hash,
            t.name,
            ...
            t.created_at
        FROM torrents t
        WHERE t.name ILIKE first_pattern
        LIMIT 10000
    ),
    -- Filter DHT candidates for all words
    dht_filtered AS (
        SELECT dc.*
        FROM dht_candidates dc
        WHERE NOT EXISTS (
            SELECT 1 FROM unnest(search_words) AS w
            WHERE dc.name NOT ILIKE '%' || w || '%'
        )
    ),
    -- Join with metadata for filtered results only
    dht_results AS (
        SELECT
