-- Fix get_user_podcast_subscriptions to include description and website_url
-- This fixes the missing metadata in the podcast list UI
-- 
-- The previous migration failed because PostgreSQL doesn't allow changing
-- the return type of an existing function with CREATE OR REPLACE.
-- We need to drop the function first.

-- Drop the existing function first since we're changing the return type
DROP FUNCTION IF EXISTS get_user_podcast_subscriptions(UUID);

-- Recreate the function with the new return type including description and website_url
