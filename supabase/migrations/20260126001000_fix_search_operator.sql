-- Fix search_all_torrents to use ILIKE instead of % operator
-- The GIN trigram index supports ILIKE queries directly, no need for % operator
-- This avoids the search_path issue with pg_trgm operators
...
BEGIN
    ...
    user_results AS (
        ...
        FROM bt_torrents bt
        WHERE
            bt.name ILIKE like_pattern
            OR bt.clean_title ILIKE like_pattern
    ),
    -- DHT torrents - ILIKE will use the GIN trigram index
    ...
    dht_results AS (
        ...
        WHERE
            -- ILIKE with GIN trigram index is fast
            t.name ILIKE like_pattern
