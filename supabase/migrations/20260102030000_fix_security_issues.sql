...
-- 1. Remove SECURITY DEFINER from torrent_stats view
-- 2. Enable RLS on torrent_folders table
-- 3. Fix all functions with mutable search_path

-- ============================================================================
-- Fix torrent_stats view - recreate with SECURITY INVOKER
-- ============================================================================

-- Drop and recreate the view with explicit SECURITY INVOKER
DROP VIEW IF EXISTS torrent_stats;

CREATE VIEW torrent_stats
...
-- Re-grant access to the stats view
GRANT SELECT ON torrent_stats TO authenticated;
GRANT SELECT ON torrent_stats TO anon;

-- ============================================================================
-- Enable RLS on torrent_folders table
-- ============================================================================

ALTER TABLE torrent_folders ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Anyone can read torrent folders" ON torrent_folders;
DROP POLICY IF EXISTS "Service role can manage folders" ON torrent_folders;

-- Everyone can read torrent folders (folder metadata is public)
CREATE POLICY "Anyone can read torrent folders"
  ON torrent_folders FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM torrents t
      WHERE t.id = torrent_folders.torrent_id
      AND t.status = 'ready'
    )
  );

-- Service role can manage all folders (for metadata enrichment)
CREATE POLICY "Service role can manage folders"
  ON torrent_folders FOR ALL
  USING (auth.jwt() ->> 'role' = 'service_role');

-- Add comment for documentation
COMMENT ON VIEW torrent_stats IS 'Aggregate statistics for torrents - uses SECURITY INVOKER for proper RLS enforcement';
