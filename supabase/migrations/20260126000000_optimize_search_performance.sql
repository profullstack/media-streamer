-- Optimize search_all_torrents for performance
-- The ILIKE '%query%' pattern causes full table scans on millions of rows
-- This migration adds full-text search indexes and rewrites the query

-- Enable pg_trgm extension for trigram matching (if not already enabled)
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Add GIN trigram index on DHT torrents.name for fast pattern matching
CREATE INDEX IF NOT EXISTS idx_torrents_name_trgm
ON torrents USING GIN (name gin_trgm_ops);

-- Add GIN trigram index on bt_torrents.name for consistency
CREATE INDEX IF NOT EXISTS idx_bt_torrents_name_trgm
ON bt_torrents USING GIN (name gin_trgm_ops);

-- Add GIN trigram index on bt_torrents.clean_title
CREATE INDEX IF NOT EXISTS idx_bt_torrents_clean_title_trgm
ON bt_torrents USING GIN (clean_title gin_trgm_ops);

-- Add index on seeders for faster sorting (DHT table)
CREATE INDEX IF NOT EXISTS idx_torrents_torrent_sources_seeders
ON torrents_torrent_sources (info_hash, seeders DESC NULLS LAST);
...
BEGIN
    ...
    user_results AS (
        ...
        FROM bt_torrents bt
        WHERE
            bt.name ILIKE '%' || normalized_query || '%'
            OR bt.clean_title ILIKE '%' || normalized_query || '%'
    ),
    -- DHT torrents - use trigram similarity for index usage
    -- Limit DHT results early to prevent runaway queries
    dht_results AS (
        SELECT
        ...
        WHERE
            -- Use trigram similarity - this will use the GIN index
            t.name % normalized_query
...
-- Set similarity threshold for trigram matching (0.3 is default, lower = more results)
-- This can be adjusted based on search quality needs
